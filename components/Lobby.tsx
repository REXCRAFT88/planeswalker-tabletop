...
import { Shield, Play, Plus, Edit3, Layers, Search, X, Loader, Users, BookOpen, Save, Trash2, Check, Crown, Maximize, Download, Upload, Zap, Palette } from 'lucide-react';
...
export const Lobby: React.FC<LobbyProps> = ({
    ...
    const [customMatImage, setCustomMatImage] = useState(() => localStorage.getItem('planeswalker_mat_image') || '');
    const [customSleeveImage, setCustomSleeveImage] = useState(() => localStorage.getItem('planeswalker_sleeve_image') || '');
    const [matOffsetX, setMatOffsetX] = useState(() => Number(localStorage.getItem('planeswalker_mat_offset_x')) || 0);
    const [matOffsetY, setMatOffsetY] = useState(() => Number(localStorage.getItem('planeswalker_mat_offset_y')) || 0);
    const [matScale, setMatScale] = useState(() => Number(localStorage.getItem('planeswalker_mat_scale')) || 100);
    const [sleeveOffsetX, setSleeveOffsetX] = useState(() => Number(localStorage.getItem('planeswalker_sleeve_offset_x')) || 0);
    const [sleeveOffsetY, setSleeveOffsetY] = useState(() => Number(localStorage.getItem('planeswalker_sleeve_offset_y')) || 0);
    const [sleeveScale, setSleeveScale] = useState(() => Number(localStorage.getItem('planeswalker_sleeve_scale')) || 100);

    // Save customization settings to localStorage
    useEffect(() => {
        localStorage.setItem('planeswalker_mat_image', customMatImage);
        localStorage.setItem('planeswalker_sleeve_image', customSleeveImage);
        localStorage.setItem('planeswalker_mat_offset_x', String(matOffsetX));
        localStorage.setItem('planeswalker_mat_offset_y', String(matOffsetY));
        localStorage.setItem('planeswalker_mat_scale', String(matScale));
        localStorage.setItem('planeswalker_sleeve_offset_x', String(sleeveOffsetX));
        localStorage.setItem('planeswalker_sleeve_offset_y', String(sleeveOffsetY));
        localStorage.setItem('planeswalker_sleeve_scale', String(sleeveScale));
    }, [customMatImage, customSleeveImage, matOffsetX, matOffsetY, matScale, sleeveOffsetX, sleeveOffsetY, sleeveScale]);
        ...
        <>
                ...
                <div className="min-h-full flex flex-col items-center justify-center p-4 md:p-6 animate-in fade-in duration-700">
                    <div className="w-full max-w-md">
                        <div className="absolute top-4 right-4">
                            ...
                            <button onClick={() => setShowCustomizeModal(true)} className="ml-2 p-2 bg-purple-900 rounded-full text-white shadow-lg border border-purple-700" title="Customize Playmat & Sleeve">
                                <Palette size={20} />
                            </button>
                        </div>
            ...
            {showCustomizeModal && (
                <div className="fixed inset-0 z-[12000] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
                    <div className="bg-gray-900/95 border border-white/20 rounded-2xl p-6 shadow-2xl max-w-2xl w-full animate-in zoom-in-95 duration-200">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-500">Customize Playmat & Sleeve</h3>
                            <button onClick={() => setShowCustomizeModal(false)} className="p-2 hover:bg-white/10 rounded-full transition-colors"><X size={20} className="text-gray-400" /></button>
                        </div>
                        ...
                        <div className="flex gap-2 mb-6 border-b border-gray-700">
                            <button
                                onClick={() => {}}
                                className="px-4 py-2 bg-purple-600 border-b-2 border-purple-400 text-white font-bold"
                            >
                                Playmat
                            </button>
                            <button
                                onClick={() => {}}
                                className="px-4 py-2 text-gray-400 border-b-2 border-transparent hover:text-gray-300 font-bold"
                            >
                                Card Sleeve
                            </button>
                        </div>

                        {/* Playmat Customization */}
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-bold text-gray-400 uppercase tracking-wider mb-2">Playmat Image URL</label>
                                <input
                                    type="text"
                                    value={customMatImage}
                                    onChange={(e) => setCustomMatImage(e.target.value)}
                            ...
                            {/* Mat Preview with Positioning Controls */}
                            <div className="bg-gray-800 rounded-xl p-4 border border-gray-700">
                                <h4 className="text-sm font-bold text-gray-300 uppercase tracking-wider mb-3">Preview & Positioning</h4>
                                <div className="flex gap-4">
                                    <div className="flex-1 relative">
                                        <div className="w-48 h-32 rounded-lg border-2 border-white/20 overflow-hidden bg-gray-900 relative">
                                            <div
                                                className="absolute inset-0 transition-transform duration-200"
                                                style={{
                                                    backgroundImage: customMatImage ? `url(${customMatImage})` : undefined,
                                                    backgroundPosition: 'center',
                                                    backgroundSize: `${matScale}%`,
                                                    transform: `translate(${matOffsetX}px, ${matOffsetY}px)`
                                                }}
                                            />
                                        </div>
                                        <div className="absolute -top-2 -right-2 bg-white px-2 py-1 rounded shadow-lg text-[10px] font-bold z-10">
                                            {customMatImage ? 'Custom Mat' : 'Default Mat'}
                                        </div>
                                    </div>

                                    <div className="w-64 space-y-3">
                                        <div>
                                            <label className="block text-xs text-gray-400 mb-1">Horizontal Offset</label>
                                            <div className="flex items-center gap-2">
                                                <input
                                                    ...
                                                    max="200"
                                                    value={matOffsetX}
                                                    onChange={(e) => setMatOffsetX(Number(e.target.value))}
                                                    className="flex-1"
                                                />
                                                <span className="text-xs text-gray-300 w-12">{matOffsetX}px</span>
                                            </div>
                                        </div>
                                        <div>
                                            <label className="block text-xs text-gray-400 mb-1">Vertical Offset</label>
                                            <div className="flex items-center gap-2">
                                                <input
                                                    type="range"
                                                    min="-200"
                                                    max="200"
                                                    value={matOffsetY}
                                                    onChange={(e) => setMatOffsetY(Number(e.target.value))}
                                                    className="flex-1"
                                                />
                                                <span className="text-xs text-gray-300 w-12">{matOffsetY}px</span>
                                            </div>
                                        </div>
                                        <div>
                                            ...
                                            <div className="flex items-center gap-2">
                                                <input
                                                    ...
                                                    max="200"
                                                    value={matScale}
                                                    onChange={(e) => setMatScale(Number(e.target.value))}
                                                    className="flex-1"
                                                />
                                                <span className="text-xs text-gray-300 w-12">{matScale}%</span>
                                            </div>
                                        </div>
                                        <button
                                            onClick={() => {
                                            ...
                                            className="w-full mt-2 p-2 bg-gray-700 hover:bg-gray-600 text-gray-300 text-xs font-bold rounded"
                                        >
                                            Reset Position
                                        </button>
                                    </div>
                            ...
                            <div>
                                <button
                                    onClick={() => setCustomMatImage('')}
                                    className="px-3 py-2 bg-red-900/50 hover:bg-red-900 text-red-200 text-sm font-bold rounded"
                                >
                                    Clear Custom Mat
                                </button>
                            </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input
                                    type="text"
                                    value={customSleeveImage}
                                    onChange={(e) => setCustomSleeveImage(e.target.value)}
                                    placeholder="https://example.com/sleeve.jpg"
                                    className="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-2 text-white focus:border-purple-500 outline-none transition-colors"
                                />
                            </div>

                            {/* Sleeve Preview with Positioning Controls */}
                            <div className="bg-gray-800 rounded-xl p-4 border border-gray-700">
                                <h4 className="text-sm font-bold text-gray-300 uppercase tracking-wider mb-3">Preview & Positioning</h4>
                                <div className="flex gap-4">
                                    <div className="flex-1 relative">
                                        <div className="w-20 h-28 rounded-lg border-2 border-white/20 overflow-hidden bg-gray-900 relative">
                                            <div
                                                className="absolute inset-0 transition-transform duration-200"
                                                style={{
                                                    backgroundImage: customSleeveImage ? `url(${customSleeveImage})` : undefined,
                                                    backgroundPosition: 'center',
                                                    backgroundSize: `${sleeveScale}%`,
                                                    transform: `translate(${sleeveOffsetX}px, ${sleeveOffsetY}px)`
                                                }}
                                            />
                                        </div>
                                    </div>
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-2">
                                                <input
                                                    ...
                                                    max="50"
                                                    value={sleeveOffsetX}
                                                    onChange={(e) => setSleeveOffsetX(Number(e.target.value))}
                                                    className="flex-1"
                                                />
                                                <span className="text-xs text-gray-300 w-12">{sleeveOffsetX}px</span>
                                            </div>
                                        </div>
                                        <div>
                                            <label className="block text-xs text-gray-400 mb-1">Vertical Offset</label>
                                            <div className="flex items-center gap-2">
                                                <input
                                                    ...
                                                    max="50"
                                                    value={sleeveOffsetY}
                                                    onChange={(e) => setSleeveOffsetY(Number(e.target.value))}
                                                    className="flex-1"
                                                />
                                                <span className="text-xs text-gray-300 w-12">{sleeveOffsetY}px</span>
                                            </div>
                                        </div>
                                        <div>
                                            <label className="block text-xs text-gray-400 mb-1">Scale</label>
                                            <div className="flex items-center gap-2">
                                                <input
                                                    ...
                                                    className="flex-1"
                                                />
                                                <span className="text-xs text-gray-300 w-12">{sleeveScale}%</span>
                                            </div>
                                        </div>
                                        <button
                                            onClick={() => {
                        ...
                        <button
                            ...
                            className="w-full mt-8 py-3 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white font-bold rounded-xl shadow-lg transition-all active:scale-95"
                        >
                            Apply Changes
                        </button>
                    </div>
